...
exports.authenticate = function (req, options) {
  ...
  request.get(self._authorizationURL)
    ...
    .end(function done(err, res) {
      ...
      const sessionKey = res.body.session_key;

      var data;
      try {
        var pc = new WXBizDataCrypt(self._appid, sessionKey);
        data = pc.decryptData(req.query.encryptedData, req.query.iv);
      } catch (e) {
        return self.fail('data decrypt error');
      }


      var profile = data;// Profile.parse(res.body);
      profile.id = data.unionId || data.openId;
      profile.picture = data.avatarUrl;
      profile.displayName = data.nickName;
      profile.accessToken = sessionKey;
